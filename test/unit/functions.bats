...
@test "releaser::get_last_version_from_changelog if 1st line has no version" {
...
@test "releaser::get_last_version_from_changelog if no version whatsoever" {
...
@test "releaser::get_last_version_from_whole_changelog if 1st line has no version" {
...
@test "releaser::get_last_version_from_whole_changelog if no version whatsoever" {
...
@test "releaser::get_last_git_tagged_version fails if no git repo" {
...
@test "get_last_git_tagged_version fails if no git repo" {
  run /bin/bash -c "source ${releaser} && cd /tmp && get_last_git_tagged_version"
  assert_output --partial "Not a git repository"
  assert_equal "$status" 1
}
...
@test "releaser::bump_patch_version fails if no major part" {
...
@test "releaser::bump_patch_version fails if no minor part" {
  run /bin/bash -c "source ${releaser} && releaser::bump_patch_version \"129..11\""
  assert_output --partial "Version was not SemVer"
  assert_equal "$status" 1
}
...
@test "releaser::bump_patch_version fails if no patch part" {
  run /bin/bash -c "source ${releaser} && releaser::bump_patch_version \"129.11\""
  assert_output --partial "Version was not SemVer"
  assert_equal "$status" 1
}
...
@test "releaser::publish_to_archive fails if endpoint_directory_name not set" {
  run /bin/bash -c "source ${releaser} && RELEASER_LOG_LEVEL=debug dryrun=true releaser::publish_to_archive"
...
@test "releaser::publish_to_archive fails if version not set" {
  run /bin/bash -c "source ${releaser} && RELEASER_LOG_LEVEL=debug dryrun=true releaser::publish_to_archive \"my_endpoint_name\""
...
@test "releaser::publish_to_archive fails if file_to_publish not set" {
  run /bin/bash -c "source ${releaser} && RELEASER_LOG_LEVEL=debug dryrun=true releaser::publish_to_archive \"my_endpoint_name\" \"0.1.123\""
...
@test "releaser::publish_to_archive succeeds if all set" {
  run /bin/bash -c "source ${releaser} && RELEASER_LOG_LEVEL=debug dryrun=true releaser::publish_to_archive \"my_endpoint_name\" \"0.1.123\" \"CHANGELOG.md\""
