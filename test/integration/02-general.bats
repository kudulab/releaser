...
@test "get_next_oversion" {
  ...
  run /bin/bash -c "cd ${ide_docker_image_dir} && source ${releaser} && releaser::get_next_oversion 2>/dev/null"
  assert_output "0.1.0"
  assert_equal "$status" 0
}
...
@test "get_next_oversion can be saved to bash variable" {
  ...
  assert_output "0.1.0"
  assert_equal "$status" 0
  version=$(cd ${ide_docker_image_dir} && source ${releaser} && releaser::get_next_oversion)
  run echo "${version}"
  assert_output "0.1.0"
  assert_equal "$status" 0
}
...
@test "set_next_oversion" {
  ...
  run /bin/bash -c "cd ${ide_docker_image_dir} && source ${releaser} && releaser::set_next_oversion \"41.111.3\""
  assert_output --partial "Set next_version into Consul for docker-releaser-test: 41.111.3"
  assert_equal "$status" 0
  /bin/bash -c "cd ${ide_docker_image_dir} && source ${releaser} && releaser::set_next_oversion \"0.1.0\""
}
...
@test "publish_to_archive succeeds if all set 2" {
  run /bin/bash -c "source ${releaser} && RELEASER_LOG_LEVEL=debug releaser::publish_to_archive \"releaser-test\" \"0.1.123\" \"test/integration/test-files/file-to-publish\""
  assert_output --partial "Published into rsync://rsync.archive.ai-traders.com/archive/releaser-test/0.1.123"
  assert_equal "$status" 0
